...
public class DemoOne : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		if( GUILayout.Button( "Add Rotation Recognizer" ) )
		{
			...
			float w = cube.GetComponent <BoxCollider2D> ().size.x;
			float h = cube.GetComponent <BoxCollider2D> ().size.y;
			float x = cube.transform.position.x;
			float y = cube.transform.position.y;

			recognizer.gestureRecognizedEvent += ( r ) =>
			{
				float z = cube.transform.eulerAngles.z;
				if (z >= 270f || z == 0) {
					cube.Rotate( Vector3.back , recognizer.deltaRotation);
				}
				if (z > 0 && z < 90f) {
					Vector3 desAng = new Vector3 (0,0,0);
					Vector3 smooth = Vector3.Lerp (desAng, cube.transform.eulerAngles, Time.deltaTime);
					cube.transform.eulerAngles = smooth;
				}
				if (z < 270f && z > 180f) {
